...
def publish(
    ...
    if oss.type in ('local', 'fake'):
        ...
        if IS_WINDOWS:
            ...
            bat_2_exe(
                ...
                icon=(
                    manifest['launcher']['icon'] or paths.build.launcher_icon
                ),
                show_console=False,
            )
    ...
    print(
        ':tv4',
        'publish done. see snapshot at "<depsland>/apps/{}/{}"'.format(
...
def _check_manifest(manifest_new: T.Manifest, manifest_old: T.Manifest) -> None:
...
def _show_change(
    ...
    """
    print `<title>: <old> -> <new>` in color.
    """
    print(
        ':psr{}'.format('i1' if show_index else ''),
